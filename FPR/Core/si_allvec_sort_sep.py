...
#SIを行うクラス
class SI_all_vec():
    def __init__(self, X, t, result_dim, B, dimension, sortIndex, Sigma=None):
        self.X = X
        ...
        self.Sigma = Sigma
        if(Sigma is None):
            self.Sigma = np.kron(np.identity(self.d),sigma*np.identity(self.N))
        else:
            self.Sigma = np.kron(Sigma, sigma*np.identity(self.N))
    ...
    def inference(self):
        ...
        for i in range(1, len(self.t)-1):
            for k in self.result_dim[i-1]:
                ...
                eta_t.append(eta_k)
                c.append(np.reciprocal(np.dot(np.dot(eta_k,self.Sigma),eta_k)) * np.dot(self.Sigma,eta_k))
                ...
                naive_p_value = norm.cdf(np.dot(eta_k, self.vec_X) / (np.sqrt(self.sigma) * np.sqrt(1/(self.t[i] - self.t[i-1])+1/(self.t[i+1] - self.t[i]))))
                naive_p_values.append(1-naive_p_value)
        ...
        for index, interval in enumerate(quadra_interval):
            selective_p_values.append(1 - common_function.tn_cdf(np.dot(eta_t[index],self.vec_X), interval, var=np.dot(np.dot(eta_t[index], self.Sigma),eta_t[index])))

        return selective_p_values, naive_p_values
